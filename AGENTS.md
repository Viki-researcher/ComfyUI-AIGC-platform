...
This is a **Data Generation Platform** (数据生成平台) monorepo — **current version: v0.4**.
...
- **PostgreSQL must be started manually**: After VM startup, run `sudo pg_ctlcluster 16 main start` before launching the backend. The `data_generation` database and `postgres` user (password: `postgres`) are already created in the snapshot.
...
- **Backend uses a `.venv`**: The backend Python deps live in `vue-fastapi-admin-main/.venv`. Use `.venv/bin/python3` (or `.venv/bin/ruff`, `.venv/bin/uvicorn`) to run backend tools. If deps were installed globally via pip, they may land in `~/.local/bin` — ensure that is on `PATH` if needed.
- **Pre-existing lint issues**: Both frontend (10 prettier/prettier formatting errors) and backend (6 ruff errors, including 4 F841 unused-variable warnings) have pre-existing lint issues that are not blockers.
...
- **Frontend proxy error detection (v0.3+)**: The Axios error handler in `src/utils/http/error.ts` distinguishes proxy errors (backend unreachable, non-JSON 500 response) from real backend 500s. Proxy errors show "后端服务不可用". Do not change `getErrorMessage(500)` without also updating the proxy-error detection branch.
- **Image preview gallery (v0.4+)**: The workbench "预览图像" button opens an ElDialog with a gallery viewer supporting left/right arrow keyboard navigation, click-to-navigate buttons, and thumbnail strip click. The backend also provides `GET /api/projects/{id}/browse` as an HTML directory listing (no auth required).

### Lint commands

- Frontend: `cd art-design-pro && pnpm lint`
- Backend: `cd vue-fastapi-admin-main && .venv/bin/ruff check ./app`

### Docs

Detailed setup and deployment docs are in `docs/` (Chinese):
- `docs/00_README.md` — Document index and module overview
...
- `docs/05_功能模块文档.md` — Feature module documentation
- `docs/10_平台启动与停止.md` — Platform start/stop scripts
- `docs/11_ComfyUI_启动逻辑说明.md` — ComfyUI startup logic
- `docs/20_系统架构与功能总览.md` — System architecture & full feature overview
- `docs/21_API接口完整规范.md` — Full REST API specification
