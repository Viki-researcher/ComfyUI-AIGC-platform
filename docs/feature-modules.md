...
- [一阶段功能](#一阶段功能)
  ...
  - [1.2 多模态能力](#12-多模态能力)
  - [1.3 模型管理](#13-模型管理)
  - [1.4 Skills 技能系统](#14-skills-技能系统)
  - [2.1 Agent 框架](#21-agent-框架)
  - [2.3 数据分析 Agent](#23-数据分析-agent)
  - [3. RAG 增强](#3-rag-增强)
  - [5. 项目管理增强 (target_count)](#5-项目管理增强-target_count)
...
Skills（技能）是预配置的 LLM 提示词模板，用户在 AI 聊天窗口中一键调用，无需手动编写提示词。
...
| `workflow_helper`  | ComfyUI 工作流助手 | AI 生成 | 帮助理解和优化 ComfyUI 工作流参数      |

#### 使用方式

在 AI 聊天窗口输入区域，点击魔术棒图标（✨）打开技能选择面板。选择技能后输入框上方显示当前激活的技能标签，发送消息时自动调用对应技能 API。
...
在 `vue-fastapi-admin-main/app/services/skills_config.py` 的 `SKILLS` 列表中追加 `SkillConfig` 实例即可。详见 [skills-config.md](skills-config.md)。
...
### 5. 项目管理增强 (target_count)

#### 目标生成数量

项目新增 `target_count` 字段，用于控制每个项目的生成配额：

- **创建项目时**可指定 `target_count`（0 = 不限制）
- **打开 ComfyUI 时**后端检查：若 `generated_count >= target_count`（且 target_count > 0），返回错误提示"已达到目标生成数量上限"
- **项目列表**返回 `target_count` 和 `generated_count`，前端可展示进度条
