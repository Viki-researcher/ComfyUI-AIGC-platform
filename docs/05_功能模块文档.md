...
项目卡片上的操作按钮现为「预览图像」（原「打开图像目录」已替换）。点击后打开 ElDialog 弹窗，内嵌图片画廊浏览器，支持：

- **左右键盘箭头导航**：按 `←` / `→` 切换上一张/下一张图片
- **点击导航按钮**：弹窗内左右两侧的 `‹` / `›` 按钮
- **底部缩略图条**：显示所有图片缩略图，点击可直接跳转到对应图片
- **图片计数器**：顶部显示当前位置和文件名（如 `3 / 25 — img_00003.png`）

后端同时提供目录浏览页面：

```
GET /api/projects/{id}/browse
```

返回 HTML 目录列表页面（无需登录鉴权），可在浏览器中直接打开查看项目输出目录下的所有文件。
...
| 指标卡片 | 数据来源 | 说明 |
|----------|----------|------|
| 今日生成量 | `GenerationLog` 按 `created_at`（Asia/Shanghai 时区）统计今日记录数 | 对比昨日数量 |
