...
> 最后更新：v0.4.0 — 合并 AI 对话/Agent/RAG/工作流编辑器 + 仪表盘时区修复 + 图片预览 + 前端错误检测。
...
| 后端 | FastAPI + Tortoise ORM + Aerich + Uvicorn | 端口 8989 |
...
> **v0.4 时区修复**：「今日生成量」使用 `created_at` 字段按 `Asia/Shanghai` 时区判定当天范围，确保跨日统计准确。
...
前端 Vite 代理（`/api` → 后端 8989）内置连接检测：当后端服务不可达时，前端拦截代理错误并显示 **「后端服务不可用」** 提示，而非通用的「服务器内部错误」，帮助用户快速定位问题。
