...
| 后端       | `vue-fastapi-admin-main/`  | 8989    | FastAPI 异步服务                |
...
| 预览图像   | `GET /api/projects/{id}/browse`        | HTML 页面列出项目已生成的图片目录              |

- **target_count**：项目目标生成数量，0 = 不限制。当 `generated_count >= target_count` 时，后端拒绝启动 ComfyUI 并提示已达上限。
- **预览图像**（v0.4.0）：项目卡片上的"预览图像"按钮打开 ElDialog 图片画廊，支持键盘左右箭头导航浏览已生成的图片。
...
> **时区处理**（v0.4.0）：Dashboard 统计基于 `created_at` 字段，前端使用 `Asia/Shanghai` 时区进行日期分组和展示，确保统计数据与用户本地时间一致。
...
- **代理错误检测**（v0.4.0）：前端在 API 请求失败时检测后端代理错误，若后端服务不可用则显示"后端服务不可用"提示，引导用户检查服务状态，避免展示原始网络错误。

### 3.10 LLM 配置

- **API Key 回退机制**（v0.4.0）：`Settings.model_post_init` 在启动时检测无效的 API Key（长度 < 8、纯数字、"123"、"test" 等占位值），自动回退到 `.env` 文件中的配置。此机制可防止 CI/CD 或 Cloud Agent 注入的占位符值覆盖有效配置。详见 [llm-chat-config.md](llm-chat-config.md)。
...
| GET    | `/api/projects/{id}/browse`            | 预览图像（HTML 页面）      |
