...
class SAM3UnifiedPromptSegmentation:
    """
    Unified multi-prompt segmentation using SAM3.

    组合文本、框、点、掩码多种 prompt，一次性完成目标分割并输出
    masks / visualization / boxes / scores。

    提示：
    - 文本 + 正/负框 + 点：走 SAM3 的 grounding 流程（类似 SAM3Grounding）
    - mask_prompt 以及点约束也会在推理后用于过滤和裁剪结果（兼容旧行为）
    """
        ...
        return {
            ...
            "optional": {
                "text_prompt": ("STRING", {
                    ...
                    "multiline": False,
                    "placeholder": "e.g., 'cat', 'person in red', 'car'",
                    "tooltip": "Describe what to segment using natural language"
                }),
                "positive_boxes": ("SAM3_BOXES_PROMPT", {
                    "tooltip": "Positive boxes to focus detection on specific regions (from SAM3CombineBoxes)"
                }),
                "negative_boxes": ("SAM3_BOXES_PROMPT", {
                    "tooltip": "Negative boxes to exclude regions (from SAM3CombineBoxes)"
                }),
                "boxes_prompt": ("SAM3_BOXES_PROMPT", {
                    "tooltip": "Extra boxes that will be treated as positive boxes"
                }),
                "points_prompt": ("SAM3_POINTS_PROMPT", {
                    "tooltip": "Generic points prompt; all points视为前景点用于结果过滤"
                }),
                "positive_points": ("SAM3_POINTS_PROMPT", {
                    "tooltip": "Foreground points; masks must cover这些点"
                }),
                "negative_points": ("SAM3_POINTS_PROMPT", {
                    "tooltip": "Background points; masks覆盖这些点会被过滤掉"
                }),
                "mask_prompt": ("MASK", {
                    "tooltip": "Optional mask used作为硬裁剪区域（只保留该区域内的预测）"
                }),
                "use_points_as_post_filter": ("BOOLEAN", {
                    "default": True,
                    "tooltip": "为 True 时使用 points/positive_points/negative_points 作为后处理过滤条件；为 False 时仅作为 grounding 点提示使用"
                }),
                "confidence_threshold": ("FLOAT", {
                    "default": 0.2,
                    "min": 0.0,
    ...
    def segment(
        ...
        mask_prompt=None,
        use_points_as_post_filter=True,
        ...
        主要流程：
        1. 使用文本 + 正/负框 + 点运行 grounding，得到候选 masks + scores
        2. （可选）使用 points/positive_points/negative_points/mask_prompt 对候选结果做过滤与裁剪（可与 grounding 点提示同时使用）
        3. 重新根据最终 masks 计算 boxes，并生成可视化与 JSON 输出
        """
        import json
        ...
        print("[SAM3 Unified] Running unified prompt segmentation")

        # 2. 图片预处理
        pil_image = comfy_image_to_pil(image)
        img_w, img_h = pil_image.size
        print(f"[SAM3 Unified] Image size: {pil_image.size}")

        # 3. grounding 流程：文本 + 多种 box prompts + points prompts
        state = processor.set_image(pil_image)

        if text_prompt and text_prompt.strip():
            print(f"[SAM3 Unified] Text prompt: '{text_prompt.strip()}'")
            state = processor.set_text_prompt(text_prompt.strip(), state)
        ...
        def _extend_boxes(src, default_label=True):
            if src is not None and len(src.get("boxes", [])) > 0:
                all_boxes.extend(src["boxes"])
                # 如果 src 里有 labels 就用原来的，否则用 default_label
                if "labels" in src and len(src["labels"]) == len(src["boxes"]):
                    all_box_labels.extend(src["labels"])
                else:
                    all_box_labels.extend([default_label] * len(src["boxes"]))

        # positive / negative boxes（来自 SAM3CombineBoxes）
        _extend_boxes(positive_boxes, default_label=True)
        _extend_boxes(negative_boxes, default_label=False)
        # 额外 boxes_prompt 一律视为 positive
        _extend_boxes(boxes_prompt, default_label=True)

        if len(all_boxes) > 0:
            print(f"[SAM3 Unified] Adding {len(all_boxes)} box prompts...")
            state = processor.add_multiple_box_prompts(
                all_boxes,
                all_box_labels,
                state,
        ...
        # 3.2 points prompts：作为“真正的几何 prompt”喂给 grounding（类似 SAM3Segmentation 的点提示）
        all_points = []
        all_point_labels = []

        def _extend_points(src, default_label=1):
            """
            src: SAM3_POINTS_PROMPT = {"points": [[x,y],...], "labels":[1/0,...](optional)}
            default_label: fallback label when src has no usable labels
            """
            if src is None:
                return
            pts = src.get("points", [])
            if not pts:
                return
            lbs = src.get("labels", None)
            if isinstance(lbs, list) and len(lbs) == len(pts):
                for p, lb in zip(pts, lbs):
                    all_points.append(p)
                    all_point_labels.append(int(lb))
            else:
                for p in pts:
                    all_points.append(p)
                    all_point_labels.append(int(default_label))

        # points_prompt：若带 labels 则尊重 labels，否则默认全部视为正点
        _extend_points(points_prompt, default_label=1)
        # positive_points / negative_points：强制语义分别为前景/背景
        _extend_points(positive_points, default_label=1)
        _extend_points(negative_points, default_label=0)

        if len(all_points) > 0:
            pos_cnt = sum(1 for lb in all_point_labels if int(lb) == 1)
            neg_cnt = len(all_point_labels) - pos_cnt
            print(f"[SAM3 Unified] Adding point prompts to grounding: {pos_cnt} positive, {neg_cnt} negative")
        ...
        if masks is None or len(masks) == 0:
            print(
                f"[SAM3 Unified] No detections found for prompt at threshold {confidence_threshold}"
            )
            empty_mask = torch.zeros(1, img_h, img_w)
            del state
            gc.collect()
        ...
        # 5. 置信度排序
        if scores is not None and len(scores) > 0:
            print(f"[SAM3 Unified] Sorting {len(scores)} detections by score...")
            sorted_indices = torch.argsort(scores, descending=True)
            masks = masks[sorted_indices]
            scores = scores[sorted_indices]

        # 6. 根据 confidence_threshold 过滤
        if scores is not None:
            keep = scores >= confidence_threshold
            if keep.sum() == 0:
                print(
                    f"[SAM3 Unified] All detections filtered out by confidence_threshold={confidence_threshold}"
                )
                empty_mask = torch.zeros(1, img_h, img_w)
                del state
                gc.collect()
        ...
        # 7. 限制最大检测数量
        if max_detections > 0 and len(masks) > max_detections:
            print(f"[SAM3 Unified] Limiting to top {max_detections} detections")
            masks = masks[:max_detections]
            scores = scores[:max_detections] if scores is not None else None

        # 8. 使用 mask_prompt / points 做后处理过滤
        masks = masks.float()

        # 8.1 mask_prompt：作为硬裁剪区域，只保留 mask_prompt>0.5 的区域
        if mask_prompt is not None:
            ...
            if isinstance(mp, torch.Tensor):
                ...
                mp = mp.unsqueeze(0).unsqueeze(0)  # [1,1,h,w]
                mp = F.interpolate(
                    mp, size=(img_h, img_w), mode="nearest"
                )[0, 0]  # [H,W]
                mp_bin = (mp > 0.5).float()
                masks = masks * mp_bin.unsqueeze(0)

        # 8.2 点约束：只保留满足点条件的 mask（可选）
        if use_points_as_post_filter:
            all_pos_points = []
            all_neg_points = []

            def _collect_points(src, is_positive=True):
                if src is not None and len(src.get("points", [])) > 0:
                    pts = src["points"]
                    if is_positive:
                        all_pos_points.extend(pts)
                    else:
                        all_neg_points.extend(pts)

            # 通用 points_prompt 视为正点
            _collect_points(points_prompt, is_positive=True)
            _collect_points(positive_points, is_positive=True)
            _collect_points(negative_points, is_positive=False)

            if len(all_pos_points) > 0 or len(all_neg_points) > 0:
                print(
                    f"[SAM3 Unified] Applying point constraints: "
                    f"{len(all_pos_points)} positive, {len(all_neg_points)} negative"
                )

                def _point_in_mask(mask_tensor, x_norm, y_norm):
                    """检查归一化坐标点是否在 mask 内。mask 可能为 (H,W) 或 (1,H,W)，且分辨率可能与图像不同，故用 mask 自身尺寸换算坐标。"""
                    # 保证为 2D：grounding 返回的 masks 有时为 (1,H,W)
                    m = mask_tensor
                    while m.dim() > 2:
                        m = m.squeeze(0)
                    if m.dim() != 2:
                ...
                for i in range(masks.shape[0]):
                    ...
                    # 所有正点中至少有一个要落在 mask 上
                    if len(all_pos_points) > 0:
                        if not any(
                            _point_in_mask(m, pt[0], pt[1]) for pt in all_pos_points
                        ):
                            ok = False

                    # 所有负点都不能落在 mask 上
                    if ok and len(all_neg_points) > 0:
                        if any(
                            _point_in_mask(m, pt[0], pt[1]) for pt in all_neg_points
                        ):
                            ok = False
                ...
                if len(keep_indices) == 0:
                    print("[SAM3 Unified] All masks filtered out by point constraints")
                    empty_mask = torch.zeros(1, img_h, img_w)
                    del state
                    gc.collect()
                    if torch.cuda.is_available():
        ...
        # 9. 基于最终 masks 重新计算 boxes（masks 可能为 (N,1,H,W)，需先 squeeze 成 2D）
        boxes_list = []
        h_mask, w_mask = None, None
        for i in range(masks.shape[0]):
            m = masks[i]
            while m.dim() > 2:
                m = m.squeeze(0)
            if h_mask is None:
                h_mask, w_mask = m.shape[0], m.shape[1]
            mask_coords = torch.where(m > 0.5)
            if len(mask_coords[0]) > 0:
                y1 = mask_coords[0].min().item()
                y2 = mask_coords[0].max().item()
                x1 = mask_coords[1].min().item()
                ...
                boxes_list.append([0, 0, 0, 0])
        boxes = torch.tensor(boxes_list).float()
        # 若 mask 分辨率与图像不同，将 box 从 mask 坐标缩放到图像坐标（便于可视化和下游使用）
        if h_mask is not None and w_mask is not None and (h_mask != img_h or w_mask != img_w):
        ...
        print(f"[SAM3 Unified] Creating visualization...")
        vis_image = visualize_masks_on_image(
            pil_image,
            masks,
            boxes,
        ...
        boxes_py = tensor_to_list(boxes)
        scores_py = tensor_to_list(scores) if scores is not None else []

        boxes_json = json.dumps(boxes_py, indent=2)
        scores_json = json.dumps(scores_py, indent=2)

        print(f"[SAM3 Unified] Segmentation complete: {len(comfy_masks)} masks")
        ...
        # 12. 可选下卸载模型
        if offload_model:
            print("[SAM3 Unified] Offloading model to CPU to free VRAM...")
            sam3_model.unpatch_model()
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
...
NODE_CLASS_MAPPINGS = {
    ...
    "SAM3UnifiedPromptSegmentation": SAM3UnifiedPromptSegmentation,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "SAM3UnifiedPromptSegmentation": "SAM3 Unified Prompt Segmentation",
}
